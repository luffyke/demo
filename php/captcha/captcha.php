...
$chars = "23456789ABCDEFGHJKLMNPRSTWXYabcdefghjkmnpqrstuvwxy";
$vercode = "";
//随机从字符串中取得字符，并组成字符串
for($i=0; $i<5; $i++){
	//设置随机时间种子
	...
	//根据随机数，取得字符
	$vercode .= substr($chars, $rand, 1);
}
//注册一个SESSION变量，用于注册读取认证码
//session_unregister("vercode");
...
for($i=0; $i<strlen($vercode); $i++){
	//设置字的颜色
	$frontColor = ImageColorAllocate($im, rand(0,120),rand(0,120),rand(0,120));
	//把字符写入图形中
	imagestring($im,10, rand(20*$i+1,20*$i+10), rand(0,5), substr($vercode,$i,1),$frontColor);
}
//输出PNG格式图形
imagepng($im);
//释放
